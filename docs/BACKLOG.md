...
| Implementar "Desactivar usuario" (revocar acceso, conservar datos) | Hoy solo existe "Eliminar" (irreversible) — falta una forma de revocar el acceso sin borrar los datos del instructor. Plan de migración ya escrito, sin implementar (ver "Decisión" punto 4 de `docs/ADR/0008-rediseno-configuracion.md`): extender `admin_list_profiles()` para exponer el estado baneado de `auth.users`, nuevo endpoint `setUserActive.js` sobre `auth.admin.updateUserById(ban_duration)`, sin añadir ninguna tabla/columna nueva | Medio-Alto (gestión real de cuentas) | S una vez aprobado el plan | Bajo el cambio en sí (usa un campo ya existente de Supabase Auth), pero es un cambio de superficie de autenticación — requiere aprobación explícita antes de implementarse (regla de CLAUDE.md sobre cambios de autenticación/permisos) | Aprobación explícita del plan de migración descrito en el ADR |
