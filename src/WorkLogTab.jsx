  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
        <h3 className="mb-4 font-semibold text-slate-800">Nueva entrada</h3>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 px-4 py-3">
          <div className="text-sm text-slate-600">
            {preview ? (
              ...
              <span className="text-amber-600">Sin tarifa configurada para esta combinación — ve a Tarifas para añadirla.</span>
            ) : (
      ...
      <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100">
        <div className="px-5 py-3">
          <h3 className="font-semibold text-slate-800">Registros ({filteredSorted.length})</h3>
        </div>
        <ListFilterBar filters={filters} setFilters={setFilters} schoolOptions={schoolNames} activityOptions={activityNames} />
        <div className="divide-y divide-slate-100">
          {filteredSorted.length === 0 && <p className="px-5 py-6 text-sm text-slate-400">Sin registros con estos filtros.</p>}
          {filteredSorted.map((e) => {
            ...
            return (
              <div key={e.id} className="flex items-center gap-3 px-5 py-3 text-sm">
                ...
                <button onClick={() => worklog.deleteRow(e.id)} className="text-slate-300 hover:text-red-500">
                  <Trash2 size={16} />
                </button>
              </div>
            );
